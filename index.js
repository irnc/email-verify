...
module.exports.verify = function verify(email, options, callback) {
  const params = {};
  const args = (arguments.length === 1 ? [arguments[0]] : Array(...arguments));
  ...
  if (!validator(params.email)) {
    return params.callback(null, {
      success: false, info: 'Invalid Email Structure', addr: email, params, code: infoCodes.invalidEmailStructure,
    });
  }
...
function startDNSQueries(params) {
  const domain = params.email.split(/[@]/).splice(-1)[0].toLowerCase();
...
function beginSMTPQueries(params) {
  ...
  const socket = net.createConnection(params.options.port, params.options.smtp);
  ...
  if (params.options.timeout > 0) {
    socket.setTimeout(params.options.timeout, () => {
      callback(null, {
        success: false, info: 'Connection Timed Out', addr: params.email, code: infoCodes.SMTPConnectionTimeout, tryagain,
      });
      socket.destroy();
    });
  }
  ...
  socket.once('error', (err) => {
    ...
    callback(err, {
      success: false, info: 'SMTP connection error', addr: params.email, code: infoCodes.SMTPConnectionError, tryagain,
    });
  });
